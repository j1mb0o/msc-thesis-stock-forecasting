...
def plot_metrics_vs_training_days(df, ticker, timefreq, figures_root):
    ...
    for metric_to_plot in ["MAPE", "MAE", "RMSE"]:
        ...
        markers_map = {method: marker for marker, method in zip(['o', 's', '^', 'd', 'p', 'X'], model_methods)}
        ...
        for idx, horizon_val in enumerate(unique_horizons):
            ...
            for method in model_methods:
                method_data = data_for_horizon[data_for_horizon['forecasting_method'] == method].sort_values(by='training_period_value')
            ...
            unit = data_for_horizon['training_period_unit'].iloc[0] if not data_for_horizon.empty else "units"
        ...
        fig.suptitle(f"{metric_to_plot} vs. Training Data Size for {ticker} ({timefreq})", fontsize=14, y=1.03)
...
def main():
    parser = argparse.ArgumentParser(description="Generate plots for a given ticker and time frequency.")
    ...
    print(f"--- Starting Plot Generation for Ticker: {args.ticker}, Timefreq: {args.timefreq} ---")
