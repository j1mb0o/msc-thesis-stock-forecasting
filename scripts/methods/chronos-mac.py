...
class ChronosForecaster:
    ...
    def __init__(self, train_data, test_data, model_name="amazon/chronos-t5-base", horizon_len=1):
        """
    ...
    def forecast(self, num_samples=20):
        ...
        logging.info(f"Chronos forecast generated for {len(self.test_data)} total steps.")
        if len(forecasts) > self.test_data.shape[0]:
            forecasts = forecasts[:self.test_data.shape[0]]

        final_forecast = pd.Series(forecasts, index=self.test_data.index, name=f"Chronos Forecast (horizon={self.horizon})")
...
if __name__ == "__main__":
    ...
    chronos_happy = ChronosForecaster(train_data=train_main.copy(), test_data=test_main.copy(), horizon_len=horizon_happy)
    ...
    chronos_incongruent = ChronosForecaster(train_data=train_main.copy(), test_data=test_incongruent.copy(), horizon_len=horizon_incongruent)
    ...
    chronos_short = ChronosForecaster(train_data=train_main.copy(), test_data=test_short.copy(), horizon_len=horizon_large)
